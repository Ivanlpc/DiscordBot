...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('add')
                .setDescription('Añade a alguien al bot')
                .addStringOption(option => option.setName('twitch').setDescription('Nombre del canal de twitch').setRequired(true))
                .addUserOption(option => option.setName('discord').setDescription('Discord del streamer').setRequired(false)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('remove')
                .setDescription('Elimina a alguien del bot')
                .addUserOption(option => option.setName('discord').setDescription('Discord del streamer').setRequired(false))),
    async execute(interaction, client) {
        if (!interaction.member.roles.cache.has(config.rolTwitch)) return interaction.reply({  //Comprueba si el que interacciona tiene el rol de streamer de la config
